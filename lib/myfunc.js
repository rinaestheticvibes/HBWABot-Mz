 ...
 * @param {store} store 
 */
exports.smsg = (HBWABotMz, m, store) => {
  if (!m) return m;
  ...
  if (m.message) {
    ...
    if (m.quoted) {
      ...
      m.quoted.delete = () => HBWABotMz.sendMessage(m.quoted.chat, {
        delete: vM.key
      });
  ...
  m.reply = (text, chatId = m.chat, options = {}) => Buffer.isBuffer(text) ? HBWABotMz.sendMedia(chatId, text, 'file', '', m, {
    ...options
  }) : HBWABotMz.sendText(chatId, text, m, {
    ...options
  });
  m.copy = () => exports.smsg(conn, M.fromObject(M.toObject(m)));
  m.copyNForward = (jid = m.chat, forceForward = false, options = {}) => HBWABotMz.copyNForward(jid, m, forceForward, options);
  HBWABotMz.appenTextMessage = async (text, chatUpdate) => {
